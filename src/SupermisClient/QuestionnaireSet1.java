...
public class QuestionnaireSet1 extends JFrame implements ActionListener {

	Vector<SupermisClient.QuestionnaireSet1> q1 = new Vector<SupermisClient.QuestionnaireSet1>();
	...
	public void actionPerformed(ActionEvent ee) {
		...
		try{
			...
			String sql = "select Q_Questions from question where Q_ID = 1";
...
try{
	...
	RemoteQuestions i = (RemoteQuestions)reg.lookup("supermis");
			...
			while(rs.next()){
				...
				try{
					...
					String sqlq = "select U_Id from `user` where `U_Username` = '"+lbl_user.getText()+"'";
					PreparedStatement p1 = connection.prepareStatement(sqlq);
		...
		if(ee.getSource().equals(yesButton)) { 
			...
			try{
				...
				if(f==true){
					...
					pst1.setString(4, "1");
					pst1.execute();
					QuestionnaireSet2 q2 = new QuestionnaireSet2();
					q2.setVisible(true);
	...
	JButton question = new JButton("Show First Question");
